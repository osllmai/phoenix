...
int ConversationList::count() const{return m_models.count();}
...
OfflineModel* ConversationList::at(int index) const{
    if (index < 0 || index >= m_models.count())
        return nullptr;
    return m_models.at(index);
}

void ConversationList::likeRequest(const int id, const bool isLike){
    emit requestUpdateIsLikeModel(id, isLike);
}

void ConversationList::downloadRequest(const int id, QString directoryPath){
...
void ConversationList::cancelRequest(const int id){
    OfflineModel* offlineModel = findModelById(id);
    if(offlineModel == nullptr) return;
...
void ConversationList::addRequest(QString directoryPath){

}

void ConversationList::addModel(const double fileSize, const int ramRamrequired, const QString& fileName, const QString& url,
...
OfflineModel* ConversationList::findModelById(int id) {
