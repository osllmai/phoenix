...
T.Button {
    ...
    function iconColor(){
        if(!control.pressed &&!control.hovered &&(settingsBodyId.currentIndex !== numberPage))
    ...
    contentItem: Rectangle{
        id: backgroundId
        color: control.hovered || (settingsBodyId.currentIndex === numberPage) ? Style.Colors.menuHoverBackground : Style.Colors.transparent
        anchors.fill: parent
        radius: 10
        clip: true
        ...
        ToolButton {
            ...
            background: null
            icon{
                source: (settingsBodyId.currentIndex == numberPage)? control.myFillIcon : control.myIcon
