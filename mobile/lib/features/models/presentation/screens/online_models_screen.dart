...
class _OnlineModelsScreenState extends ConsumerState<OnlineModelsScreen> {
      ...
      body: RadiantBackdrop(
        child: Column(
          children: [
            OnlineHeader(
              state: state,
              onState: (s) => ref.read(onlineStateProvider.notifier).state = s,
            ),
            ...
            if (state == OnlineState.loading)
              const CloudNotice(
                text:
                    'Connecting to the IndoxHub gateway and fetching the '
                    'model catalog…',
              ),
  ...
  Widget _body(OnlineState state) => switch (state) {
    ...
    OnlineState.empty => const OnlineCenterState(
      ...
      body:
          'No hosted models match your search and filters. Clear them, '
    ...
    OnlineState.denied => const OnlineCenterState(
      ...
      errorBox:
          '401 Unauthorized — your IndoxHub API key is missing, '
          'invalid, or revoked.',
    ),
  };
}
