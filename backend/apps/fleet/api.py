...
router = Router(tags=['fleet'], auth=session_token_auth)


def _detail_qs(owner):
    return FleetRun.objects.filter(owner=owner).prefetch_related(
        Prefetch('lanes', queryset=FleetLane.objects.all()),
        Prefetch('events', queryset=FleetEvent.objects.all()),
    )


@router.get('/runs/', response=list[RunListOut])
...
@router.post('/runs/', response=RunDetailOut)
def fan_out(request, payload: FanOutIn):
    ...
    return get_object_or_404(_detail_qs(request.auth), pk=run.pk)


@router.get('/runs/{run_id}/', response=RunDetailOut)
def get_run(request, run_id: int):
    return get_object_or_404(_detail_qs(request.auth), pk=run_id)


@router.post('/runs/{run_id}/merge/', response=RunDetailOut)
def merge_winner(request, run_id: int, payload: MergeIn):
    ...
    with transaction.atomic():
        run = get_object_or_404(
            FleetRun.objects.select_for_update(), pk=run_id, owner=request.auth
        )
