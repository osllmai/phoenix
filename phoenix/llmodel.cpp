...
{
   // std::cout << std::endl << __LINE__ << "\t" << __FILE__ << "\t" << __FUNCTION__ << std::endl;
      ...
      //  std::cout << "\t" << __LINE__ << "\t";
        ...
        auto search_in_directory = [&](const std::string& paths) {
         ...
         //   std::cout << "\t" << __LINE__ << "\t";
          ...
          //      std::cout << std::endl << "Searching in directory: " << path << std::endl;

                // Iterate over all libraries
                for (const auto &f : fs::directory_iterator(u8_path)) {
            //        std::cout << "\t" << __LINE__ << "\t";
                    const fs::path &p = f.path();
                    //      std::cout << std::endl << "Checking file: " << p.filename().string() << std::endl;

                    if (p.extension() != LIB_FILE_EXT) {
                     //   std::cout << "Skipping file (wrong extension): " << p.filename().string() << std::endl;
                        continue;
                    }

               //     std::string stem_str = p.stem().string();
             //       std::cout <<std::endl<< "File stem: " << stem_str << std::endl;

                    if (!std::regex_search(p.stem().string(), re)) {
              //          std::cout << "Skipping file (regex mismatch): " << p.filename().string() << std::endl;
                        continue;
                    }

                  //  std::cout << "File matches regex: " << p.filename().string() << std::endl;
           ...
           //         std::cout << "\t" << __LINE__ << "\t";
                //    std::cout << "Adding implementation: " << p.filename().string() << std::endl;
...
LLModel *LLModel::Implementation::construct(const std::string &modelPath, const std::string &backend, int n_ctx)
{
 //   std::cout << std::endl<<__LINE__<<"\t" << __FILE__ <<"\t"<< __FUNCTION__ << std::endl;
   ...
   //     std::cout << "\t" << __LINE__ << "\t";
        ...
        //std::cout << "\n" << __LINE__ <<"\t"<< DEFAULT_BACKENDS[0] << "\t" << DEFAULT_BACKENDS[1] << "\n";
    }

    for (const auto &desiredBackend: desiredBackends) {
  //      std::cout << "\n" << __LINE__ << " modelPath: " << modelPath << "  applyCPUVariant(desiredBackend): " << applyCPUVariant(desiredBackend) << "\n";
