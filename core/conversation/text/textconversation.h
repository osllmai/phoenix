...
class TextConversation : public QObject
{
    Q_OBJECT
    QML_ELEMENT
...
public:
    explicit TextConversation(QObject* parent = nullptr)
        : QObject(parent), m_model(new Model(this)), m_modelSettings(new ModelSettings(1,this)),
        m_messageList(new MessageList(this)), m_responseList(new ResponseList(this)),
...
signals:
    ...
    void requestInsertMessage(const int conversationId, const QString &text,
                              const QString &fileName, const QString &icon, bool isPrompt, const int like);
