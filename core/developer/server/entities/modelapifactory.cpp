#include "modelapifactory.h"

ModelAPIFactory::ModelAPIFactory(const QString &scheme, const QString &hostName, int port)
    : scheme(scheme), hostName(hostName), port(port)
{}

std::optional<ChatAPI> ModelAPIFactory::fromJson(const QJsonObject &json) const
{
